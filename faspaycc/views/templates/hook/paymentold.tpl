	...
	{foreach from=$pglist item=pg}
		{if $pg.active}
			{if strpos($link->getModuleLink('faspay', 'payment'),"index.php") !== false}
				<a href="{$link->getModuleLink('faspay', 'payment')}&pg={$pg.cd}" title="{$pg.desc}" style="display:block; line-height:32px; padding-left:150px; margin: 5px 0; background:url({$this_path}icon_{$pg.id}.png) no-repeat;">{$pg.desc}</a>
			{else}
				<a href="{$link->getModuleLink('faspay', 'payment')}?pg={$pg.cd}" title="{$pg.desc}" style="display:block; line-height:32px; padding-left:150px; margin: 5px 0; background:url({$this_path}icon_{$pg.id}.png) no-repeat;">{$pg.desc}</a>
			{/if}
		{/if}
